...
public class MainActivity extends Activity
{
	...
	protected void onCreate(Bundle savedInstanceState)
	{
		 ...
		 * devices for (BluetoothDevice device : bluetooth.getBondedDevices()) {
		 * // Add the name and address to an array adapter to show in a ListView
		 * Toast.makeText(this , device.getName() , Toast.LENGTH_SHORT).show();
		 * // mArrayAdapter.add(device.getName() + "\n" + device.getAddress());
		 * } pairedDevice = (BluetoothDevice)
		 * (bluetooth.getBondedDevices().toArray()[0]);
		 * 
		 * } btServer = new BloothServiceHandler(this ,mHandler);
		 * btServer.start(); if( pairedDevice != null) {
		 * btServer.connect(pairedDevice);
		 * 
		 * } String msgToSend;
		 * 
		 * sendButton.setOnClickListener(new View.OnClickListener() { public
		 * void onClick(View v) { // Do something in response to button click //
		 ...
		 */
		/*
		 * setupCamera(); camera.takePicture(null, null, new
		 * PhotoHandler(getApplicationContext())) ;
		 */
		/*
		 * Intent serviceChoiceIntent = new
		 * Intent(getApplicationContext(),BrowserActivity.class);
		 * startActivity(serviceChoiceIntent);
		 * 
